...
5. **Wrong formatter gate.** Ruff passed, but hosted CI uses repository-native `isort` through `make format-check`. The consumer PR needed a trivial follow-up commit. Always run the repository-native format target, not a guessed substitute.
