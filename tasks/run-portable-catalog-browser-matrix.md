...
The plan load itself is also protected. Missing, malformed, empty, duplicate, or unsupported plans now emit a producer-owned catalog failure summary and trace status. When safe candidate records remain recoverable, the harness emits blocked per-form receipts without trusting the invalid plan as executable input. Summaries and form receipts record both first-failure boundary and ownership.
