...
class SbirSttrInformationTests(unittest.TestCase):
    ...
    def test_simple_conditions_compile_and_clear_remove_or_policy_rules_do_not(self) -> None:
        ...
        self.assertEqual(
            ui["#/properties/otherAgency"]["condition"],
            {"scope": "#/properties/agency/properties/value", "schema": {"const": "Other"}},
        )
        self.assertEqual(
            ui["#/properties/phaseIIAwardsReceived/properties/value"]["condition"]["schema"],
            {"enum": ["SBIR", "Both"]},
        )
        self.assertEqual(
            ui["#/properties/nonprofitResearchPartnerUei"]["condition"]["schema"],
            {"enum": ["STTR", "Both"]},
        )
